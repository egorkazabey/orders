...
import { formatPriceRange, isAcceptingOrders, isBusinessOpenNow } from '@/entities/business'
...
export function ProductsTab({ onOrder }: ProductsTabProps) {
	...
	const acceptingOrders = isAcceptingOrders(business.schedule, business.orderCutoffMinutes)
	const openNow = isBusinessOpenNow(business.schedule)
	...
	return (
		<div className="flex flex-col gap-9">
			{!acceptingOrders && (
				<p className="rounded-2xl bg-stone-100 px-4 py-3 text-center text-sm font-medium text-stone-600 dark:bg-stone-800 dark:text-stone-300">
					{openNow
						? 'Blížíme se k zavírací době, nové objednávky již nepřijímáme.'
						: 'Obchod je momentálně zavřený, objednávky nejsou možné.'}
				</p>
			)}
					...
					<div key={category.id}>
						...
						<div className="flex flex-col gap-3">
							{categoryProducts.map((product) => {
								const disabled = product.soldOut || !acceptingOrders
								return (
									<button
										key={product.id}
										type="button"
